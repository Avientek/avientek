...
(function () {
	...
	function toggle_gst_fields(frm) {
		var hide = frm.doc.company !== INDIAN_COMPANY;
		var grid = frm.fields_dict.items && frm.fields_dict.items.grid;
		if (!grid) return;

		// Hide column in grid list view
		grid.update_docfield_property("gst_treatment", "hidden", hide ? 1 : 0);

		// Hide field inside every open row form
		grid.grid_rows.forEach(function (row) {
			if (row.grid_form && row.grid_form.fields_dict.gst_treatment) {
				row.grid_form.fields_dict.gst_treatment.df.hidden = hide ? 1 : 0;
				row.grid_form.fields_dict.gst_treatment.refresh();
			}
		});
	}
	...
	["Quotation", "Sales Order", "Sales Invoice", "Delivery Note",
	 ...
	 "Purchase Receipt", "Supplier Quotation"].forEach(function (dt) {
		frappe.ui.form.on(dt, {
			refresh: toggle_gst_fields,
			company: toggle_gst_fields,
		});

		// Also trigger when a child row form is opened
		frappe.ui.form.on(dt + " Item", {
			form_render: function (frm) {
				toggle_gst_fields(frm);
			}
		});
	});
