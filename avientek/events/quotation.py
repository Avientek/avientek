...
# ──────────────────────────────────────────────────────────────
# 4)  AVERAGE MARGIN LOOK-UP  (unchanged)
# ──────────────────────────────────────────────────────────────
def get_avg_margin_percent_from_sales_orders(brand, salesperson):
    if not (brand and salesperson):
        return 0.0

    # Get submitted Sales Orders of that salesperson and matching brand items
    quotation_items = frappe.db.sql("""
        SELECT qi.rate, qi.custom_cogs
    ...
    for row in quotation_items:
        ...
        cogs = flt(row.cogs)
        if rate > 0:
            margin_percent = (rate - cogs) / rate * 100
            frappe.errprint(f"[MARGIN DEBUG] Rate: {rate}, COGS: {cogs}, Margin: {margin_percent}")
            margins.append(margin_percent)
            frappe.errprint(margins)
...
def validate_margin_before_submit(doc, method):
    ...
    bu_head_required = False

    for row in doc.items:
        brand = row.brand
        std_margin = flt(row.std_margin_per)
        custom_margin = flt(row.custom_margin_ or 0)

        critical_margin = std_margin - (0.2 * std_margin)

        # Condition 1: Custom margin >= standard margin
        if custom_margin >= std_margin:
            frappe.errprint(f"{brand} approved: custom_margin ({custom_margin}) >= std_margin ({std_margin})")
            continue

        # Condition 2: Avg margin >= standard margin
        avg_margin = get_avg_margin_percent_from_sales_orders(brand, salesperson)
        frappe.errprint(f"Brand: {brand}, Std Margin: {std_margin}, Custom Margin: {custom_margin}, Avg Margin: {avg_margin}")

        if avg_margin >= std_margin:
            frappe.errprint(f"{brand} approved: avg_margin ({avg_margin}) >= std_margin ({std_margin})")
            continue

        # Condition 3: Custom margin >= critical margin → Approve
        if custom_margin >= critical_margin:
            frappe.errprint(f"{brand} approved: custom_margin ({custom_margin}) >= critical_margin ({critical_margin})")
            continue

        # If margin below critical margin → BU Head approval required
        bu_head_required = True
        margins_failed.append({
            "brand": brand,
            "std_margin": std_margin,
            "custom_margin": custom_margin,
            "avg_margin": avg_margin
        })
        frappe.errprint(f"{brand} requires BU Head approval: custom_margin ({custom_margin}) < critical_margin ({critical_margin})")

    if bu_head_required:
        doc.workflow_state = "BU Head Approval Required"
        frappe.throw("Items have critically low margin. Director approval required. Cannot submit.")
    else:
        doc.workflow_state = "Approved"

# ──────────────────────────────────────────────────────────────
# 6)  VALIDATE  (runs every Save; does all maths)
# ──────────────────────────────────────────────────────────────
